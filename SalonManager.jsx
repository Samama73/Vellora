    ...
    <div style={{ minHeight: "100vh", display: "flex", background: C.ivory, fontFamily: fontSans }}>
      ...
      <div style={{ flex: 1, background: `linear-gradient(155deg, ${C.plum} 0%, #241823 100%)`, color: C.goldLight, display: "flex", flexDirection: "column", justifyContent: "center", padding: "5vw", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          <Scissors size={26} color={C.gold} />
          <span style={{ fontFamily: fontVoice, fontSize: 22, fontWeight: 500, letterSpacing: 0.5 }}>Vellora</span>
        </div>
    ...
    <div style={{ minHeight: "100vh", display: "flex", background: C.ivory, fontFamily: fontSans, color: C.ink }}>
      ...
      <aside style={{ width: 230, background: C.plum, color: "#fff", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          {visibleNav.map((n) => {
            ...
            return (
              <button key={n.id} onClick={() => setTab(n.id)} style={{
                ...
                marginBottom: 5, borderRadius: 14, border: "none", cursor: "pointer", fontSize: 13.5,
        ...
        <div style={{ padding: 16, borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <button onClick={handleLogout} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "1px solid rgba(255,255,255,0.2)", color: "#D9CFDA", padding: "8px 10px", borderRadius: 12, fontSize: 12.5, cursor: "pointer", width: "100%", justifyContent: "center" }}>
      ...
      <main style={{ flex: 1, padding: "28px 36px", overflowY: "auto", minWidth: 0 }}>
        {loadError && (
          <div style={{ ...card, borderColor: C.red, marginBottom: 18, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {tab === "dashboard" && <Dashboard appts={appts} inventory={inventory} employees={employees} />}
        {tab === "appointments" && <Appointments appts={appts} setAppts={setAppts} setLoadError={setLoadError} />}
        {tab === "marketing" && <Marketing user={user} />}
        {tab === "accounts" && <Accounts salaries={salaries} setSalaries={setSalaries} employees={employees} setLoadError={setLoadError} />}
        {tab === "team" && <Team employees={employees} setEmployees={setEmployees} inventory={inventory} setInventory={setInventory} setLoadError={setLoadError} />}
      </main>
    </div>
  );
}
...
/* ================= DASHBOARD ================= */
function Dashboard({ appts, inventory, employees }) {
...
function Appointments({ appts, setAppts, setLoadError }) {
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {[...appts].sort((a, b) => b.date.localeCompare(a.date)).map((a) => (
          <div key={a.id} style={{ ...card, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 18px", flexWrap: "wrap", gap: 10 }}>
            <div>
...
/* ================= ACCOUNTS ================= */
function Accounts({ salaries, setSalaries, employees, setLoadError }) {
  const [saving, setSaving] = useState(false);
...
/* ================= TEAM & INVENTORY ================= */
function Team({ employees, setEmployees, inventory, setInventory, setLoadError }) {
